...
#### Hint:
R looks for specified files (such as `"data/Sulis2023_fig3G.tsv"`) relative to its so-called `working directory`. 
To make sure it finds the data file, set the folder containing the seminar files as your working directory by clicking on `Session` and `Set working directory to source file location`. 
...
Also, perhaps give it another colour scheme?
